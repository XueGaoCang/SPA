        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(p, index) in skuSaleAttrValueList" :key="p.id">
                <dt class="title">
                  {{ p.saleAttrName }}
                </dt>
                <dd changepirce="90">
            ...
            <div class="cartWrap">
              <div class="controls">
                <input
                  ...
                  @change="changeskNum"
                />
                <a href="javascript:" class="plus" @click="skNum++">+</a>
                <a
                  href="javascript:"
                  class="mins"
                  @click="skNum > 1 ? skNum-- : (skNum = 1)"
                  >-</a
                >
                <!-- 如果skNum>1了则可以减一，若skNum小于等于1了则skNum就等于1 -->
              </div>
              <div class="add">
                <a href="javascript:" @click="shop">加入购物</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    changeskNum() {
      let value = this.skNum * 1;
      if (isNaN(value) || value < 1) {
        this.skNum = 1;
      } else {
        this.skNum = parseInt(value);
      }
    },
    async shop() {
      //此处派发了一个actions,也向服务器发请求调用了postShop这个函数,来判断加入购物车是成功还是失败，在进行相关操作
      //this.$store.dispatch("postShop",skuId: this.$route.params.skuId,skuNum: this.skNum,});
      //上述代码为向服务器发请求调用了postShop这个函数,此方法加上asyc await 则返回的一定是Promise
      //而Promise要么成功|要么失败
      try {
        await this.$store.dispatch("postShop", {
        ...
        sessionStorage.setItem("SKUINFO", JSON.stringify(this.skuInfo)),
          this.$router.push({
            name: "addCartSuccess",
            query: {
              URL: this.skuInfo.skuDefaultImg,
              Desc: this.skuInfo.skuDesc,
              Num: this.skNum,
            },
          });
      } catch (error) {
